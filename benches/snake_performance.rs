...
fn benchmark_food_generation(c: &mut Criterion) {
    ...
    for snake_size in [3, 10, 50, 100, 200] {
        ...
        group.bench_with_input(BenchmarkId::new("snake_size", snake_size), &snake, |b, snake| {
            b.iter(|| {
                black_box(GameState::generate_food_position(snake))
            })
        });
    }
    group.finish();
...
fn benchmark_collision_detection(c: &mut Criterion) {
    ...
    for snake_size in [3, 10, 50, 100] {
        ...
        group.bench_with_input(BenchmarkId::new("snake_size", snake_size), &game, |b, game| {
            b.iter(|| {
...
fn benchmark_snake_growth(c: &mut Criterion) {
    ...
    for initial_size in [3, 10, 20, 50] {
        ...
        group.bench_with_input(BenchmarkId::new("initial_size", initial_size), &snake, |b, snake| {
            b.iter(|| {
...
fn benchmark_edge_case_scenarios(c: &mut Criterion) {
    ...
    let edge_scenarios = vec![
        ("left_edge", Position::new(0, GRID_HEIGHT / 2), Direction::Left),
        ("right_edge", Position::new(GRID_WIDTH - 1, GRID_HEIGHT / 2), Direction::Right),
        ("top_edge", Position::new(GRID_WIDTH / 2, 0), Direction::Up),
        ("bottom_edge", Position::new(GRID_WIDTH / 2, GRID_HEIGHT - 1), Direction::Down),
    ];
    
    for (name, head_pos, direction) in edge_scenarios {
        group.bench_function(name, |b| {
            b.iter(|| {
                let snake = vec![head_pos, Position::new(head_pos.x, head_pos.y + 1), Position::new(head_pos.x, head_pos.y + 2)];
                let mut game = GameState {
                    snake,
                    direction,
